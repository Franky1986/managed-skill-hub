...
- `bootstrapSkill`: the recommended `use-skill-hub` starter skill for this
  registry instance. It includes `readUrl`, `packageUrl`, `manifestUrl`,
  `versionsUrl`, `version`, and whether the served content is the built-in
  `0.0.0-initial` fallback.
...
For first-time setup, prefer `bootstrapSkill.packageUrl`. It returns a ZIP
containing `SKILL.md` plus generated `skill-hub-meta.json`, so the local agent
can later check versions and submit updates with the original registry context.
For short one-off usage, `bootstrapSkill.readUrl` can be read directly, but it
does not create local metadata.
...
- Package downloads always return ZIP, even for single-file skills.
- Every package contains generated `skill-hub-meta.json`.
- After download, verify manifest entrypoint and all manifest file paths before executing locally.
- For one-off direct reads, use `GET /api/skills/<skill-id>/files/<file-id>`
  instead of the package route.
...
## Download metadata and updates

`skill-hub-meta.json` is generated by ManagedSkillHub when a package is
downloaded. It is local consumer metadata, not authored skill content.

Agents use it to find:

- registry identity and API base URL;
- skill ID, version, UUIDs, and content digest;
- live links for discovery, manifest, versions, package, and proposals;
- proposal defaults for creating a new version;
- local update rules.

When the user asks whether a local skill has updates, read
`skill-hub-meta.json`, fetch the live `links.versions` or `links.manifest`,
compare version UUID and content digest, inspect local changes, explain the
risk, create a backup under the local skill folder, and ask before replacing
files. A missing `edited_locally` marker does not prove that the user made no
changes.

When submitting a locally edited downloaded skill back to the registry, remove
`skill-hub-meta.json` from the proposal package, read live `/howToPropose`, use
`proposalDefaults.targetSkillId`, and prefer `create_new_version`. Updates to
existing skills and `use-skill-hub` bootstrap changes require human admin
review and are not auto-published.
