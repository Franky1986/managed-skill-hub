...
Implemented - static bearer phase; the original generated setup-script design
was superseded by EPIC-012 agent-session delegation.
...
deployment in discovery responses and agent-session URLs. They are not
...
## Consumer Auth And Agent Bootstrap
...
The implemented consumer experience is the EPIC-012 agent-session page for
static bearer areas or OIDC Device Authorization for OIDC areas.

### Historical Local Credential-Store Proposal

The following local credential-store design was considered during the static
bearer phase but is not the current runtime contract. It remains here as design
history for possible non-browser clients:

- a small local setup page, CLI, native prompt, or agent-adjacent tool could ask
  for registry alias, registry URL, read token, and optional proposal token
...
The originally planned downloadable setup script was removed. EPIC-012 provides
the implemented human-in-the-loop flow: `/discover` advertises the absolute
`/frontend/agent-auth` URL, the human enters bearer tokens outside chat, and the
agent receives a short-lived area-scoped code used as
`Authorization: AgentSession <code>`. The flow never writes a user-global token
file and never asks users to paste bearer tokens into chat.
...
- `/discover` advertises agent-session delegation when a bearer area is active.
...
- Add consumer auth guidance that uses the agent-session page or OIDC Device
  Authorization without embedding tokens.
...
- `docs/howTo/README.md` or a dedicated how-to links agent-session/OIDC guidance.
- Agent-session behavior is documented for PM/non-repo consumers.
...
- agent-session endpoint/page docs and tests
