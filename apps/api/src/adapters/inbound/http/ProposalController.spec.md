...
- Reject uploaded files whose basename is `skill-hub-meta.json`, regardless of
  subfolder path. The file is generated by the registry for downloaded local
  packages and must not be re-ingested as authored proposal content.
...
- Reserved generated metadata filename -> 422
...
- A local re-proposal from a downloaded package must omit
  `skill-hub-meta.json`; uploads containing it fail before storage writes.
