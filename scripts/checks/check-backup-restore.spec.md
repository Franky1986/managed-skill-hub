...
`scripts/checks/check-backup-restore.ts` proves that repository backup and restore
...
- `scripts/operations/backup.sh` creates a tar.gz archive for an isolated `DATA_DIR`.
- `scripts/operations/restore.sh` can restore that archive with `MSH_SKIP_STOP=true`.
- `scripts/operations/restore.sh` validates archive members before restore and rejects
