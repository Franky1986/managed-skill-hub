...
mkdir -p "${SOURCE_DIR}/scripts/deployment" "${SOURCE_DIR}/.tmp" "$FAKE_BIN_DIR"
printf '%s\n' 'NODE_ENV=production' > "${SOURCE_DIR}/.env"
printf '%s\n' "JWT_SECRET='not-a-real-secret-value-for-test'" > "${DEPLOYMENT_ROOT}/persistent.secrets"

cat > "${SOURCE_DIR}/scripts/deployment/install_and_start.sh" <<'SCRIPT'
#!/usr/bin/env bash
set -euo pipefail
printf 'start:%s:%s:%s\n' \
  "$1" \
...
cat > "${SOURCE_DIR}/scripts/deployment/restart-server.sh" <<'SCRIPT'
#!/usr/bin/env bash
set -euo pipefail
...
chmod +x \
  "${SOURCE_DIR}/scripts/deployment/install_and_start.sh" \
  "${SOURCE_DIR}/scripts/deployment/restart-server.sh"
...
MSH_START_SCRIPT=release/scripts/deployment/install_and_start.sh
MSH_RUNTIME_SCRIPT=release/scripts/deployment/restart-server.sh
...
MSH_SECRETS_FILE=persistent.secrets
MSH_RUNTIME_SCRIPT=linked-release/scripts/deployment/restart-server.sh
ENV
