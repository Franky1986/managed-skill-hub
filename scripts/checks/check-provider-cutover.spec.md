...
- Path: `scripts/checks/check-provider-cutover.ts`
- Execution: explicit local invocation or `RUN_MYSQL_FULL_CHECK=true ./scripts/full-check.sh`
- Infrastructure: local MySQL stack from `scripts/development/start-mysql-stack.sh up`
...
6. Assert `scripts/development/restart-all.sh` contains the MySQL auto-start/preflight path so the operator does not need a separate manual MySQL start step.
