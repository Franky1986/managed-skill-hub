...
MySQL mode expects the local MySQL stack from `scripts/development/start-mysql-stack.sh up` or an equivalent MySQL instance configured through `MYSQL_HOST`, `MYSQL_PORT`, `MYSQL_DATABASE`, `MYSQL_USER`, and `MYSQL_PASSWORD`.
...
- Verifies `scripts/development/restart-all.sh` contains local MySQL auto-start guidance through `start-mysql-stack.sh up`.
...
- Production database migration validation; cutover is covered by `scripts/checks/check-provider-cutover.ts`.
