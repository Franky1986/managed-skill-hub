...
if [ "${MSH_SKIP_STOP:-false}" != "true" ] && [ -x "${PROJECT_ROOT}/scripts/restart-server.sh" ]; then
  bash "${PROJECT_ROOT}/scripts/restart-server.sh" stop || true
fi
...
echo "[INFO] Stack mit 'bash scripts/restart-server.sh' neu starten."
