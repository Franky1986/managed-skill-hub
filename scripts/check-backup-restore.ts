...
async function main(): Promise<void> {
  ...
  const backup = await execFileAsync('bash', ['scripts/backup.sh'], {
    cwd: root,
  ...
  const mysqlGuard = await execFileAsync('bash', ['scripts/backup.sh'], {
    cwd: root,
    env: {
      ...process.env,
  ...
  const restore = await execFileAsync('bash', ['scripts/restore.sh', archive], {
    cwd: root,
